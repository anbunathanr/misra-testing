...
function FileUploadMISRA() {
  ...
  const handleFiles = useCallback((fileList: FileList | null) => {
    ...
    const fileCount = Math.min(fileList.length, 50) // Max 50 files
    ...
    if (fileList.length > 50) {
      alert('Maximum 50 files allowed. Only first 50 files will be added.')
    }
  ...
  return (
    <Box>
      ...
      >
        <input
          type="file"
          id="misra-file-upload"
          multiple
          accept={allowedExtensions.join(',')}
          onChange={handleChange}
          style={{ display: 'none' }}
        />
        <label htmlFor="misra-file-upload" style={{ cursor: 'pointer' }}>
          <CloudUploadIcon sx={{ fontSize: 64, color: 'primary.main', mb: 2 }} />
          <Typography variant="h6" gutterBottom>
            Drag and drop C/C++ files here
          </Typography>
                    ...
                    <Box>
                      ...
                      {fileItem.status === 'success' && (
                        <Typography variant="caption" color="success.main" sx={{ display: 'block', mt: 0.5 }}>
                          Analysis queued - check results page
                        </Typography>
                      )}
                    </Box>
                  }
...
export default FileUploadMISRA
