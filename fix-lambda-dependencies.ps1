...
Write-Host "`n1️⃣  Creating Lambda Layer structure..." -ForegroundColor Yellow
...
Write-Host "2️⃣  Copying dependencies to layer..." -ForegroundColor Yellow
Copy-Item -Path "packages/backend/node_modules/*" -Destination $nodeModulesDir -Recurse -Force

# Step 3: Create zip file
Write-Host "3️⃣  Creating layer zip file..." -ForegroundColor Yellow
...
Write-Host "4️⃣  Uploading layer to AWS..." -ForegroundColor Yellow
...
Write-Host "`n5️⃣  Attaching layer to all Lambda functions..." -ForegroundColor Yellow
...
# Step 6: Wait for updates to complete
Write-Host "`n6️⃣  Waiting for Lambda updates to propagate..." -ForegroundColor Yellow
...
try {
  ...
  Write-Host "    Response: $($resp | ConvertTo-Json -Depth 1)" -ForegroundColor Gray
}
catch {
...
try {
  ...
  $resp = Invoke-RestMethod -Uri "$api/auth/register" -Method POST -Body $body -ContentType "application/json" -ErrorAction Stop
  Write-Host "  ✓ Auth endpoint works!" -ForegroundColor Green
  Write-Host "    Response: $($resp | ConvertTo-Json -Depth 1)" -ForegroundColor Gray
}
catch {
  Write-Host "  ✗ Auth endpoint failed: $($_.Exception.Response.StatusCode)" -ForegroundColor Red
  Write-Host "    Error: $($_.Exception.Message)" -ForegroundColor Red
}

# Step 8: Cleanup
Write-Host "`n8️⃣  Cleaning up temporary files..." -ForegroundColor Yellow
Remove-Item -Recurse -Force $layerDir
Remove-Item $zipPath
Write-Host "✓ Cleanup complete" -ForegroundColor Green

Write-Host "`n✅ Lambda dependencies fixed!" -ForegroundColor Green
Write-Host "`nNext steps:" -ForegroundColor Cyan
Write-Host "1. Build frontend: cd packages/frontend; npm run build" -ForegroundColor White
Write-Host "2. Deploy frontend to S3" -ForegroundColor White
Write-Host "3. Test end-to-end flow" -ForegroundColor White
